...
from attentions import CA, SimAm
...
class CNBlock(nn.Module):
    def __init__(
        ...
        # 注意力机制可在此处扩展
        # self.caAttention = attentions.CA.CoordAtt(dim, dim)  # 可选的CA注意力模块


        self.block = nn.Sequential(
            nn.Conv2d(dim, dim, kernel_size=7, padding=3, groups=dim, bias=True),
            # 注意力机制可在此处扩展
            # x = self.caAttention(x)                                         # 可选的注意力处理
            # SimAm.simam_module(dim),
            Permute([0, 2, 3, 1]),
            norm_layer(dim),
...
if __name__ == '__main__':
    ...
    net = ConvNeXt(block_setting).to('cuda')
    # pre_net = torchvision.models.convnext_tiny()
    # pre_net.load_state_dict(torch.load(rf'../pre_model/convnext_tiny.pth'))
    #
    # w = {}
    # for k, v in pre_net.state_dict().items():
    ...
    net.load_state_dict(torch.load(r'C:\Data\broncho_classification\pre_model\convnext_tiny.pth'), strict=False)
